...
(function (window, document) {
  ...
  var container, galleryContainer, currentIndex = 0, startPoint = {x: 0, y: 0, time: new Date().getTime()};
  ...
  function initDOM() {
    ...
    container.innerHTML = '<div class="preview-image-container" id="preview-image-container">'
      + generateContainer() +
      '</div><div class="optional-render"></div>';
  ...
  function handlerTouchEnd(event) {
    ...
    if (touch.clientX - startPoint.x < 0 - opts.offset && currentIndex !== opts.urls.length - 1) {
      ...
      var distance = Math.pow(startPoint.x - touch.clientX, 2) + Math.pow(startPoint.y - touch.clientY, 2);
